...
if (!empty($_POST)) {
    ...
    $seodesc = isset($_POST['seodesc']) ? $_POST['seodesc'] : '';
    $seokeyword = isset($_POST['seokeyword']) ? $_POST['seokeyword'] : '';


    // Insert new record into the blog table
    $stmt = $pdo->prepare('INSERT INTO blog VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([$id, $img, $category, $title, $content, $seotitle, $seodesc, $seokeyword]);
...
<div class="content update">
    ...
    <form action="create-blog.php" method="post">
        ...
        <input type="text" name="seotitle" placeholder="SEO Title" id="seotitle">

        <label for="seodesc">SEO Description</label>
        <input type="text" name="seodesc" placeholder="SEO Description" id="seodesc">

        <label for="seokeyword">SEO Keywords</label>
        <input type="text" name="seokeyword" placeholder="SEO Keywords" id="seokeyword">
